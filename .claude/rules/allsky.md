...
- A fit's own numbers can lie (issue #93: 606 matches over 60 frames at RMS 10.9 px rated Good, and every match was a coincidence — 1.04x chance at the 16.5 px final tolerance, RMS = tol/√2). Every fit now persists `final_tol_px` and `chance_ratio` on the model (0.0 = unknown, legacy files keep their rating), `calibration_fit_merit.fit_is_credible` judges them (`CREDIBLE_RMS_FRACTION` 0.55 carries the measured table; `CHANCE_MARGIN` 2.0), `model_quality` caps a non-credible fit at Preliminary, and rule 2's evidence bypass requires the candidate to be credible. The incumbent is re-scored on the live buffer every refinement run (`incumbent_chance.score_incumbent`, one match pass at the final tolerance on the `_RefineWorker` thread, skipped under 40 median detections); two consecutive chance-level scores discredit it for the badge, the `misaligned` caution and rule 3 — never for the file, which is not rewritten. Only the guided solve itself is exempt from all of it (`model_admission.is_user_anchored`: guided provenance, no recorded chance ratio, one image) — `is_guided` is a basin claim that every same-basin automatic refinement inherits, and those fits are judged like any other; rule 3's guided exemption keeps the basin-lock semantics and stays on `is_guided`.
