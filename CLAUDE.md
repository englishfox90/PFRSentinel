...
| `test_static_lights.py` | 15 | `static_lights` — pier lights found across a night and stripped from both pools, Polaris and a slewing light never listed, chance hits on a dense pool are not lights (presence rule), duck-typed equipment map, `clean_pools` on buffer + ring |
...
| `test_exposure_midpoint.py` | 15 | `exposure_midpoint` — SDK start + half exposure, FITS `DATE-OBS`, receipt time minus half an exposure, never raises |
| `test_pole_from_rotation.py` | 21 | `pole_from_rotation` — pole within 25 px on both rig presets and the southern mirror with Polaris hidden and a LED by the pole, same answer with three static lights and a slewing light, wrong sky-circle seed recovered, mirror flips `east_left`, `sigma_px` covers the error in ≥ 90 % of 20 seeded runs, random and static-only fields withheld, resource budget (≤ 10 s, ≤ 50 MB on 36 × 200) |
