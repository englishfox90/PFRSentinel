...
| `test_camera_auto_recovery.py` | 17 | `camera_auto_recovery` off — one plain reconnect, no ladder/USB/SDK reset (`camera_reconnect`), capture loop stops instead of retrying (cold-open quirk still absorbed, handle released on the fatal exit), controller schedules no restart or USB reset; on = unchanged |
