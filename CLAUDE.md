...
| `test_observing_window.py` | 47 | `observing_window` — twilight gate, roof gate and its opt-out, roof must read Closed on two consecutive frames, several callers in one frame count once, a reprocess of the same capture never advances or clears the count, nor does a caller with no roof verdict (Watch mode's overlay render); issue #93 rules in order: static blocks, exposure floor (0.06 s blocks, 10 s passes, absent passes, 0 disables, short frames leave the roof streak alone), Open + 3 no-star frames blocks and 2 do not, ML off blocks too, `stars_visible` vetoes a low count, recovery needs 2 frames at 2× the floor, a Closed streak still blocks with plenty of stars, blind callers and reprocesses neither count nor clear, reason cached with the verdict, one INFO line per transition, `_ML_RESULTS` untouched so the ASCOM FSM still certifies SAFE and the meteor roof rule sees Open |
