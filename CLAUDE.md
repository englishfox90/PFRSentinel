...
| `test_label_tool_dataset.py` | 12 | `ml.dataset_files` / `ml.label_suggestion` / `ml.calibration_store` — `_`-prefixed housekeeping folders never shadow live calibration JSONs, NINA > AI > CNN roof priority and the source-agreement rule, closed roof carries no sky, atomic save |
| `test_label_tool_concurrency.py` | 11 | Labeling tool — eight concurrent writers to one calibration JSON all land, a label saved during an AI call survives it, cancelled AI worker stops without waiting on in-flight requests, QThread joined before its reference is dropped, batch confirm never overwrites a human label, AI result refreshes the frame that was sent, unreadable JSON explains itself on Save & Next (offscreen Qt) |
