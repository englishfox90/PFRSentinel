...
- **The roof must not be reported closed.** If [ML Models](ML-Models) are enabled and the roof classifier reports **Closed** on two frames in a row, the overlay and calibration pause. A single Closed frame is ignored (from the next release; in 3.7.7 and earlier one frame was enough, which could blank the overlay for a frame when the exposure changed). Rigs with no roof (for example an open-air all-sky camera) can turn off **Skip Sky Features When Roof Closed** in the ML settings so a misread "Closed" does not suppress them.
