...
def test_release_body_states_the_risks_and_the_source():
    notes = load_notes()
    body = notes.release_body("3.7.7-dev.14", SHA, "main", "48",
...
def test_ref_stands_in_when_no_pr():
    body = load_notes().build_comment("3.7.7-dev.14", SHA, "my-branch", None, "https://run.invalid")
...
def test_cli_treats_an_empty_pr_as_none(capsys):
    notes = load_notes()
    assert notes.main(["comment", "--version", "3.7.7-dev.14", "--sha", SHA, "--ref", "main", "--pr", "",
